...
```{r modelRandomForest}
set.seed(1000)
# using random forest with caret was taking too long to run on my system hence used randomForest func directly, tried diff   values for its parameters 
...
```{r gbm}
set.seed(1000)
# using gbm with caret was taking too long to run on my system hence used gbm func directly with diff values for its parameters
...
*Best Model was random forest based on outsample accuracy hence used the same for final prediction of the actual test dataset (new_test). All predictions were correct.*
